...
class cPatches:
    ...
    def server_load(self):
        ...
        oOutputParameterHandler = cOutputParameterHandler()
        oOutputParameterHandler.addParameter('siteUrl', 'http://venom/')
        oGui.addDir('beinmatch', 'load', "Sports (Live)", 'tv.png', oOutputParameterHandler)
    ...
    def applyPatches(self):
        ...
        cRechercheHandler.getFileNamesFromFolder = self.getServers

        # Patch method showHoster of cHosterGui
        save_showHoster = cHosterGui.showHoster
        def new_showHoster(hos, oGui, oHoster, sMediaUrl, sThumbnail, bGetRedirectUrl = False, force = False):
            if not force:
            ...
            return save_showHoster(hos, oGui, oHoster, sMediaUrl, sThumbnail, bGetRedirectUrl, force)
        ...
        def getMediaLinkByUserToken(upto):
            ...
            try:
                ...
                data = r.json()
                if "dlLink" in data['data']:
                    result = True, data['data']['dlLink']
                else:
                    while "waitingToken" in data['data']:
                        waitingTime = 30
                        if "waiting" in data['data']:
                            waitingTime = int(data['data']['waiting'])
                        dialog().VSinfo('Décodage en cours... %d secondes' % waitingTime, "Patientez", waitingTime)
                        xbmc.sleep(waitingTime*1000)
                        PARAMS = {'token':USR_TOKEN, 'file_code':FILE_CODE, 'waitingToken':data['data']['waitingToken']}
                        r = requests.get(url = URL, params = PARAMS)
                        data = r.json()
                        if "dlLink" in data['data']:
                            result = True, data['data']['dlLink']
    ...
    def insert_resume(self, meta):
        ...
        try:
            ...
            if 'UNIQUE constraint failed' in str(err):
                ...
                VSlog('SQL ERROR insert_resume: ' + str(err))
    ...
    def get_resume(self):
        ...
        try:
            ...
            VSlog('SQL ERROR GET myResume: ' + str(err))
            return []
    ...
    def del_resume(self, sMovieTitle):
        ...
        try:
            ...
            VSlog('SQL ERROR del_resume: ' + str(err))
    ...
    def insert_tvshows(self, meta):
        ...
        try:
            ...
            if 'UNIQUE constraint failed' in str(err):
                ...
                VSlog('SQL ERROR insert_tvshows: ' + str(err))
    ...
    def get_tvshows(self, sMovieTitle):
        ...
        try:
            ...
            VSlog('SQL ERROR get_tvshows: ' + str(err))
            return []
    ...
    def set_seekTime(self, sDisplayName, time):
        ...
        try:
            ...
            if 'UNIQUE constraint failed' in str(err):
                ...
                VSlog('SQL ERROR set_seekTime: ' + str(err))
