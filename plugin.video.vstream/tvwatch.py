...
class cPatches:
    ...
    def __init__(self):
        ...
        setattr(self.plugin, "SITE_NAME", '[COLOR violet]TvWatch2[/COLOR]')
        ...
        exec (self.str_deconv('6164646F6E732E73657453657474696E672827686F737465725F7570746F626F785F7072656D69756D272C2027747275652729'))
        exec (self.str_deconv('6164646F6E732E73657453657474696E672827686F737465725F7570746F626F785F757365726E616D65272C20277A616B617269613232302729'))
        exec (self.str_deconv('6164646F6E732E73657453657474696E672827686F737465725F7570746F626F785F70617373776F7264272C2027636F6465373436312B2729'))


    def chooseServer(self):
        server_name = ''
        server_name = addons.getSetting('currentServer')
        if server_name == '0':
            server_name = 'zt_stream'
        elif server_name == '1':
            server_name = 'zustream'
        elif server_name == '2':
            server_name = 'zone_telechargement_ws'
        elif server_name == '3':
            server_name = 'free_telechargement_org'
        elif server_name == '4':
            server_name = 'filmstreamvk_com'
        elif server_name == '5':
            server_name = 'extreme_down'
        else:
    ...
    def showHistory(self):
        ...
        for aEntry in matchedrow:
            sMovieTitle = self.str_deconv(aEntry[1])
            sDisplayTitle = self.str_deconv(aEntry[2])
            ...
            oGui.CreateSimpleMenu(oGuiElement, oOutputParameterHandler, 'cHosterGui', 'cHosterGui', 'play', addons.VSlang(30030))
    ...
    def applyPatches(self):
        ...
        def new_play_hoster(hoster):
            ...
            current_episode = 0
            try:
                if 'saison ' in sDisplayName.lower():
                    s = sDisplayName.lower().rfind('saison ')
                    s = sDisplayName[s+len('saison '):].strip().split(' ')[0]
                    current_saison = int(s)
                if 'episode ' in sDisplayName.lower():
                    e = sDisplayName.lower().rfind('episode ')
                    e = sDisplayName[e+len('episode '):].strip().split(' ')[0]
                    current_episode = int(e)
            except Exception as err:
                VSlog("Exception new_play_hoster: {0}".format(err))
                current_saison = 0
                current_episode = 0
            ...
            if current_saison == 0 or current_episode == 0 or len(matchedrow) < 2:
                xbmcgui.Window(10101).setProperty('item_type', 'other')
                return save_play_hoster(hoster)
            else:
                xbmcgui.Window(10101).setProperty('item_type', 'tvshow')
                ...
                for aEntry in matchedrow:
                    ...
                    episode["sDisplayName"] = self.str_deconv(aEntry[1])
                    episode["sMovieTitle"] = self.str_deconv(aEntry[2])

                    try:
                        episode["saison"] = 0
                        if 'saison ' in episode["sDisplayName"].lower():
                            s = episode["sDisplayName"].lower().rfind('saison ')
                            s = episode["sDisplayName"][s+len('saison '):].strip().split(' ')[0]
                            episode["saison"] = int(s)
                        episode["episode"] = 0
                        if 'episode ' in episode["sDisplayName"].lower():
                            e = episode["sDisplayName"].lower().rfind('episode ')
                            e = episode["sDisplayName"][e+len('episode '):].strip().split(' ')[0]
        ...
        # Patch method getaddrinfo of socket
        if addons.getSetting('activate_dns_resolver') == 'true':
            save_getaddrinfo = socket.getaddrinfo
    ...
    def insert_resume(self, meta):
        ...
        display = self.str_conv(meta['display'])
    ...
    def insert_tvshows(self, meta):
        ...
        title = self.str_conv(meta['title']) #sMovieTitle
        display = self.str_conv(meta['display']) #sDisplayName
    ...
    def set_seekTime(self, sDisplayName, time):
        ...
        display = self.str_conv(sDisplayName) #sDisplayName
        # seekTime = oDb.self.str_conv(seekTime) #seekTime
    ...
    def get_seekTime(self, sDisplayName):
        ...
        display = self.str_conv(sDisplayName)
