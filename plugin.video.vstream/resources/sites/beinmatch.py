    ...
    for i in range(sHtmlContent.count("headerRightSide")):
        ...
        while ('"p%d"' % count) in data:
            ...
            if "أهداف وملخصات مباريات أمس" in d:
                d = "Buts et résumés de Matchs d'hier"
                icon = "replay.png"
            count += 1
            item += d + " "
...
def category():
    ...
    for i in range(contentRightSideCount):
        last = "</html>" if (i == contentRightSideCount - 1) else "contentRightSide"
    ...
    items = []
    for i in range(data.count('<table class="tabIndex">')):
        start = data.find('<table class="tabIndex">') + len('<table class="tabIndex">')
        data = data[start:]
        map = {}
