...
def showMovies(sSearch = ''):
    ...
    if (aResult[0] == True):
        ...
        for aEntry in aResult[1]:
            ...
            elif 'serie' in sUrl2 or 'anime' in sUrl2 or '-saison-' in sUrl2:
                oGui.addTV(SITE_IDENTIFIER, 'showSeriesLinks', sTitle, '', sThumb, '', oOutputParameterHandler)
...
def showSerieEpisodes():
    ...
    if (aResult[0] == True):
        ...
        total = len(aResult[1])
        for aEntry in aResult[1]:
            progress_.VSupdate(progress_, int(total/5))
            if progress_.iscanceled():
                break
            sUrl = aEntry[0]
            numEpisode = aEntry[1]
            sHosterUrl = get_protected_link(sUrl)
            if sHosterUrl and ("uptobox" in sHosterUrl.lower() or "uptostream" in sHosterUrl.lower()):
                upto = True
            elif upto:
                break
            oHoster = cHosterGui().checkHoster(sHosterUrl)
